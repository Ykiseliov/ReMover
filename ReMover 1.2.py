...
def count_files():
    ...
    return len([f for f in os.listdir(dest) if os.path.isfile(os.path.join(dest, f))])

def update_file_count():
    file_count = count_files()
    file_count_label.config(text=f"                {file_count} шт.")
...
tk.Label(app, text="Количество:").grid(row=0, column=1)
file_count_label = tk.Label(app, text=" 0 ", font=("Arial", 8))
file_count_label.grid(row=1, column=1, sticky="w")

tk.Label(app, text="Имя архива:").grid(row=0, column=0)
entry_archive_name = tk.Entry(app)
entry_archive_name.grid(row=1, column=0)

tk.Button(app, text="Archive", command=execute_rem, width=20, height=2).grid(row=2, column=0)
tk.Button(app, text="Move", command=execute_mov, width=20, height=2).grid(row=2, column=1)
